namespace FreeSR.Gateserver.Manager.Handlers
{
    ...
    {
        public static uint Avatar1 = 1310;
        ...
        [Handler(CmdType.CmdGetLineupAvatarDataCsReq)]
        public static void OnGetLineupAvatarDataCsReq(NetSession session, int cmdId, object data)
        {
            var characters = new uint[] { Avatar1, Avatar2, Avatar3, Avatar4 };
            var response = new GetLineupAvatarDataScRsp
            {
                Retcode = 0,
            };
            foreach (uint id in characters)
            {
                response.AvatarDatas.Add(new LineupAvatarData
                {
                    AvatarType = AvatarType.AvatarFormalType,
                    Hp = 10000,
                    Id = id,
            ...
            session.Send(CmdType.CmdGetLineupAvatarDataScRsp, response);
        }
